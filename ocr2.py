...
def extract_images_from_pdf(pdf_path):
    ...
    for page_num in range(len(pdf_document)):
        ...
        for img_index, img in enumerate(images):
            ...
            image_ext = base_image["ext"]  # Check the image extension

            # Only process supported image formats (e.g., 'png', 'jpeg')
            if image_ext in ["png", "jpeg", "jpg"]:
                try:
                    ...
                    print(f"Skipping unidentified image on page {page_num + 1}, image index {img_index + 1}")
            else:
                print(f"Skipping unsupported image format {image_ext} on page {page_num + 1}, image index {img_index + 1}")
